<?php

namespace App\Http\Controllers\Admin;

use App\Http\Controllers\Controller;
use App\Models\User;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Hash;
use Illuminate\Validation\Rule;
use Illuminate\Support\Facades\Cache;

class ReviewerController extends Controller
{
    public function index(Request $request)
    {
        $query = User::role('reviewer')->latest();

        if ($request->filled('q')) {
            $searchTerm = $request->q;
            $query->where(function($q) use ($searchTerm) {
                $q->where('name', 'like', "%{$searchTerm}%")
                  ->orWhere('email', 'like', "%{$searchTerm}%")
                  ->orWhere('expertise', 'like', "%{$searchTerm}%");
            });
        }

        $reviewers = $query->paginate(15)->withQueryString();

        return view('admin.reviewers.index', compact('reviewers'));
    }

    public function create()
    {
        return view('admin.reviewers.create');
    }

    public function store(Request $request)
    {
        $validated = $request->validate([
            'name' => 'required|string|max:255',
            'email' => 'required|string|email|max:255|unique:users',
            'password' => 'required|string|min:8',
            'expertise' => 'nullable|string|max:255',
            'is_active' => 'boolean',
        ]);

        $validated['password'] = Hash::make($validated['password']);
        $validated['is_active'] = $request->has('is_active');

        $user = User::create($validated);
        $user->assignRole('reviewer');

        Cache::forget('admin_reports_stats');

        return redirect()->route('admin.reviewers.index')->with('success', 'Reviewer berhasil ditambahkan.');
    }

    public function edit(User $reviewer)
    {
        return view('admin.reviewers.edit', compact('reviewer'));
    }

    public function update(Request $request, User $reviewer)
    {
        $validated = $request->validate([
            'name' => 'required|string|max:255',
            'email' => ['required', 'string', 'email', 'max:255', Rule::unique('users')->ignore($reviewer->id)],
            'password' => 'nullable|string|min:8',
            'expertise' => 'nullable|string|max:255',
            'is_active' => 'boolean',
        ]);

        if (!empty($validated['password'])) {
            $validated['password'] = Hash::make($validated['password']);
        } else {
            unset($validated['password']);
        }

        $validated['is_active'] = $request->has('is_active');

        $reviewer->update($validated);

        Cache::forget('admin_reports_stats');

        return redirect()->route('admin.reviewers.index')->with('success', 'Data reviewer berhasil diperbarui.');
    }

    public function destroy(User $reviewer)
    {
        $reviewer->delete();
        Cache::forget('admin_reports_stats');
        return redirect()->route('admin.reviewers.index')->with('success', 'Reviewer berhasil dihapus.');
    }
}
