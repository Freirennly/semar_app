<x-layouts.app :title="'Manajemen Reviewer'">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-text-secondary mt-1">Kelola daftar reviewer dan data keahlian mereka.</p>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-left text-text-muted text-[10px] uppercase tracking-widest border-b border-border bg-bg/30">
                        <th class="px-6 py-4 font-bold min-w-[200px]">Reviewer</th>
                        <th class="px-6 py-4 font-bold">Keahlian (Expertise)</th>
                        ...
                        <td class="px-6 py-4 text-right whitespace-nowrap">
                            <div class="flex items-center justify-end gap-3">
                                ...
                                <form method="POST" action="{{ route('admin.reviewers.destroy', $reviewer) }}" class="inline">
                                    @csrf @method('DELETE')
                                    <button type="button" class="text-text-muted hover:text-danger transition-colors" onclick="event.preventDefault(); window.confirmModal('Hapus reviewer {{ $reviewer->name }} secara permanen?', this.closest('form'));" title="Hapus">
