                        ...
                        <td class="px-6 py-4 text-right whitespace-nowrap">
                            <div class="flex items-center justify-end gap-3">
                                ...
                                <form method="POST" action="{{ route('admin.secretariat.destroy', $staff) }}" class="inline">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="text-text-muted hover:text-danger transition-colors" onclick="return confirm('Hapus anggota sekretariat {{ $staff->name }} secara permanen?')" title="Hapus">
