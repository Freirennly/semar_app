<x-layouts.app :title="'Edit Sekretaris'">
    <div class="mb-12 flex items-center gap-3 animate-fade-in">
        ...
        <div>
            <h1 class="text-3xl md:text-[36px] font-bold text-text tracking-tight leading-[1.3]">Edit Sekretaris</h1>
            <p class="text-sm font-medium text-text-secondary mt-2 leading-[1.2]">Perbarui data profil dan status staf sekretariat.</p>
        </div>
    </div>

    <div class="card p-6 border-primary/10 shadow-sm max-w-3xl mb-12">
        <form method="POST" action="{{ route('admin.secretariat.update', $secretariat) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="name" id="name" required class="input-field" value="{{ old('name', $secretariat->name) }}">
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" required class="input-field" value="{{ old('email', $secretariat->email) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="position" id="position" class="input-field" value="{{ old('position', $secretariat->position) }}">
            ...
            <div class="pt-2">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $secretariat->is_active) ? 'checked' : '' }} class="w-5 h-5 text-primary rounded border-border-strong focus:ring-primary focus:ring-offset-0">
