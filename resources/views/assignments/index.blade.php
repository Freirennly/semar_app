    ...
    <div class="card mb-4 p-6">
        <div class="flex items-center justify-between mb-3">
            <div class="min-w-0">
                ...
                <p class="text-xs text-text-muted mt-0.5">{{ $sub->code }} · {{ $sub->student->name }} · <x-status-badge :status="$sub->status" /></p>
            </div>
        ...
        <div class="mb-3">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <div class="flex items-center gap-2 bg-soft-surface rounded-lg px-3 py-1.5 text-sm border border-primary/10">
                    ...
                    @if($a->due_at)<span class="text-xs text-text-muted">· due {{ $a->due_at->format('d M') }}</span>@endif
        ...
        @endif
        <form method="POST" action="{{ route('assignments.store', $sub) }}" class="flex flex-col sm:flex-row items-end gap-3 pt-3 border-t border-border">
            @csrf
            ...
            <div class="w-full sm:w-auto">
                ...
                <input type="date" name="due_at" class="input-field" aria-label="Deadline review">
            </div>
            <button type="submit" class="btn-primary shrink-0 w-full sm:w-auto">Assign</button>
        </form>
    </div>
    @endforeach
